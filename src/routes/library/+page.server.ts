...
import { parseCountMode, parseLibraryFilters, parseServerScope } from '$lib/server/queries/params';
...
export const load: PageServerLoad = async ({ locals, url }) => {
	...
	const filters = parseLibraryFilters(url.searchParams, locals.timeZone);
	...
	const [summary, timeline, sections] = await Promise.all([
		...
		getSections(locals.account.id, parseServerScope(url.searchParams))
	]);
